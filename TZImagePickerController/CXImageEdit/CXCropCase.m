...
//显示的裁剪框缩放比例
@property (nonatomic, assign) CGFloat showCropScale;
@end

@implementation CXCropCase

- (instancetype)initWithFrame:(CGRect)frame
                     scrollVi:(UIScrollView *)scroll
                    imageView:(UIImageView *)imageView
                    cropScale:(CGFloat)scale {
    self = [super initWithFrame:frame];
    if (self) {
        self.showCropScale = scale;
        self.scroll = scroll;
...
- (instancetype)initWithFrame:(CGRect)frame
                     scrollVi:(UIScrollView *)scroll
                    imageView:(UIImageView *)imageView
                    cropModel:(CXCropModel *)cropModel
                    cropScale:(CGFloat)scale {
    self = [super initWithFrame:frame];
    if (self) {
        self.showCropScale = scale;
...
- (void)setUpView {
    ...
    self.initialScrollZoomScale = self.showCropScale;
...
//还原
- (void)recovery {
    self.scroll.minimumZoomScale = self.showCropScale;
    self.scroll.maximumZoomScale = kMaxZoomScale;
    ...
    [UIView animateWithDuration:0.2 animations:^{
        self.scroll.zoomScale = self.showCropScale;
...
//    CGFloat maxCropWidth = (CGRectGetWidth(self.superview.frame) - kCropLeft - kCropRight);
    CGFloat maxCropWidth = kScreenWidth * self.showCropScale;
    //最大的剪裁高度
//    CGFloat maxCropHeight = (CGRectGetHeight(self.superview.frame) - kCropTop - kCropBottom);
    CGFloat maxCropHeight = kScreenHeight * self.showCropScale;
    CGRect result;
    CGFloat x;
    CGFloat y;
    CGFloat width;
...
- (UIImageView *)leftTopIgv {
    if (!_leftTopIgv) {
        ...
        _leftTopIgv.image = [UIImage tz_imageNamedFromMyBundle:@"crop_corner_left_top"];
...
- (UIImageView *)rightTopIgv {
    if (!_rightTopIgv) {
        ...
        _rightTopIgv.image = [UIImage tz_imageNamedFromMyBundle:@"crop_corner_right_top"];
...
- (UIImageView *)leftBottomIgv {
    if (!_leftBottomIgv) {
        ...
        _leftBottomIgv.image = [UIImage tz_imageNamedFromMyBundle:@"crop_corner_left_bottom"];
...
- (UIImageView *)rightBottomIgv {
    if (!_rightBottomIgv) {
        ...
        _rightBottomIgv.image = [UIImage tz_imageNamedFromMyBundle:@"crop_corner_right_bottom"];
